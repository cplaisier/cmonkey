...
cmonkey.init <- function( env=NULL, ... ) {
  ...
  set.param( "mot.weights", c( `upstream meme`=1 ) ) ##, `upstream weeder`=0.5, `upstream spacer`=1 ) ) ## Sequence and algorithm for for motif search: Optionally use different automatically computed sequences (e.g. `downstream meme`=1) or an input file (e.g. `fstfile=zzz.fst meme`=1) (csvfile too!) and motif algos (e.g. weeder, spacer, prism)
  ...
  set.param( "set.types", list(miRNA = c(file='pita_miRNA_sets.csv',cutoff=0.05)) )
  ...
  } else if ( any( mot.scaling > 0 && exists( "genome.info" ) && ! no.genome.info ) ) {
  ...
  set.param( "spacer.cmd", c( "java -Xmx1000M -Xshare:off -jar SPACER.jar -b %s -o %s %s", ## same here.
