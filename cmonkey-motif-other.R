...
## TODO: implement "prev.run" capability to skip motif finding if cluster hasn't changed (as in meme.one.cluster)
...
gadem.one.cluster <- function( k, seq.type=names(mot.weights)[1], n.motifs=2,
  ...
  bgtab <- data.frame( names( e$genome.info$bg.list[[ seq.type ]] ),
                      sapply( e$genome.info$bg.list[[ seq.type ]], "[", 1 ) )
...
cluster.tomtom.results <- function( tt.out, seq.type=names(mot.weights)[1], e.cutoff=Inf, p.cutoff=0.05, n.cutoff=3,
                                   resid.cutoff=Inf, make.pssms=T, min.size=3, k.cut=0.5, return.aligned.pssms=F,
                                   n.gene.weight=F ) { ##, plot=F ) {
  if ( is.na( e.cutoff ) ) e.cutoff <- Inf
  if ( is.na( resid.cutoff ) ) resid.cutoff <- Inf
  if ( is.na( p.cutoff ) ) p.cutoff <- Inf
  ...
  if ( ! is.infinite( e.cutoff ) || ! is.infinite( resid.cutoff ) || ! is.infinite( p.cutoff ) )
    tt.out <- subset( tt.out, e.value1 <= e.cutoff & e.value2 <= e.cutoff &
                     resid1 <= resid.cutoff & resid2 <= resid.cutoff & p.value <= p.cutoff )
