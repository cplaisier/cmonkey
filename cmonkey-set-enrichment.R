...
get.set.enrichment.scores <- function( k, set.type=NA) {
  ...
  for(set in names(enrichment.p.values)) {
    # Calculate hypergeometric for a set
    if( enrichment.sets[[set.type]][[set]]$cutoff=='discrete' ) {
      # Already a discrete set
      set.genes <- unique(enrichment.sets[[set.type]][[set]])
    } else {
      ...
      set.i = enrichment.sets[[set.type]][[set]]
      set.genes <- names(set.i)[which(as.numeric(set.i)>=set.i$cutoff)]
    }
    overlap.genes <- intersect(cluster.genes,set.genes)
    ...
    enrichment.p.values[i] <- phyper(length(overlap.genes),length(set.genes),length(all.genes.enrichment.set)-length(set.genes),length(cluster.genes),lower.tail=F)
  }
  ...
  min.set <- names(enrichment.p.values)[which(enrichment.p.value==min(enrichment.p.value))]
  ...
  if (enrichment.sets[[set.type]][[set]]$cutoff=='discrete') {
    ...
    min.genes <- unique(enrichment.sets[[set]][[min.set]])
    overlap.genes <- intersect(cluster.genes,set.genes)
    ...
    scores[set.genes] <- 0.5 # In minimum scoring set, but not in the bicluster
  ...
  scores <- scores * enrichment.p.values[min.set]
